...
public class PoiExcelCellValueVisitor {
        ...
        if (!list.isEmpty()) {
            ...
            for (FormulaReplaceTask replace : list) {
                ...
                if (replacement.contains("${column}")) {
                    replacement = replacement.replace("${column}", CellReference.convertNumToColString(cell.getColumnIndex() + 1));
                }
    ...
    protected void visitCellValueError(PoiExcelColumnBean bean, Object cell, int errorCode, CellVisitor visitor) {
        ...
        case EXCEPTION:
            ...
            throw new RuntimeException(MessageFormat.format("encount cell error. error_code={0}({1})", errorCode, error.getString()));
        }
