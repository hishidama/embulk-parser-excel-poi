...
public class TestPoiExcelParserPlugin_convertError {
    ...
    public void testConvertError_constant(String excelFile) throws Exception {
        try (EmbulkPluginTester tester = new EmbulkPluginTester()) {
            ...
            parser.addColumn("t", "timestamp").set("column_number", "A").set("format", "%Y/%m/%d").set("on_convert_error", "constant.2000/1/1");
            ...
            for (OutputRecord r : result) {
                ...
                assertThat(r.getAsTimestamp("t"), is(Instant.ofEpochMilli(new SimpleDateFormat("yyyy/MM/dd z").parse("2000/01/01 UTC").getTime())));
            }
        }
    }
}
